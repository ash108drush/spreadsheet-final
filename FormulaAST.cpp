...
public:
    ...
    virtual double Evaluate(Sheet &sheet) const = 0;
...
public:
    ...
    double Evaluate(Sheet &sheet) const override {
        double left_val = lhs_->Evaluate(sheet);
        double right_val = rhs_->Evaluate(sheet);
        double result = 1.0;
        switch (type_) {
        case Add:
...
public:
    ...
    double Evaluate(Sheet &sheet) const override {
        double oper = operand_->Evaluate(sheet);
        switch (type_) {
        case '+':
            return oper;
        case '-':
...
public:
    ...
    double Evaluate(Sheet &sheet) const override {
        // Шаг 1: проверяем валидность позиции ячейки
        if (!cell_->IsValid()) {
            throw FormulaError(FormulaError::Category::Ref);
        }

        // Шаг 2: получаем ячейку из таблицы
        Cell* cell = dynamic_cast<Cell *>(sheet.GetCell(*cell_));

        // Шаг 3: если ячейка не существует — выбрасываем ошибку
        if (cell == nullptr) {
            throw FormulaError(FormulaError::Category::Ref); // Ссылка на несуществующую ячейку
        }

        // Шаг 4: проверяем состояние ячейки
        switch (cell->State()) {
        case CellState::READY:
            // Ячейка готова — возвращаем её значение
            return cell->value;

        case CellState::COMPUTING:
            // Циклическая зависимость — ячейка уже вычисляется
            throw FormulaError(FormulaError::Category::Cycle);

        case CellState::ERROR:
            // Ячейка в состоянии ошибки — передаём ошибку дальше
            throw FormulaError(cell->error_message);

        case CellState::EMPTY:
            // Ячейка пуста — считаем это ошибкой ссылки
            throw FormulaError(FormulaError::Category::Ref);
    }
    }
...
double FormulaAST::Execute(Sheet &sheet) const {
